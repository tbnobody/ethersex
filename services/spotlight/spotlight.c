...
static spotlight_channel_t channels[SPOTLIGHT_CHANNELS] =
  { { {0, 0, 0}, {0, 0, 0}, SPOTLIGHT_MODE_NORMAL, SPOTLIGHT_NOUPDATE, SPOTLIGHT_NOUPDATE} };
...
{
	if (send_online_lwt) {
		mqtt_construct_publish_packet(mqtt_will_topic, MQTT_WILL_MESSAGE_ONLINE, sizeof(MQTT_WILL_MESSAGE_ONLINE) - 1, MQTT_RETAIN);
	...
	for (uint8_t i = 0; i < SPOTLIGHT_CHANNELS; i++)
	{
		if (channels[i].sendUpdate == SPOTLIGHT_UPDATE) {
			...
			char payload[7];
			sprintf_P(topic, PSTR("%s/get/%d/color"), spotlight_params_ram.mqtt_topic, i + 1);
			sprintf_P(payload, PSTR("%02hhX%02hhX%02hhX"), channels[i].value.r, channels[i].value.g, channels[i].value.b);

			mqtt_construct_publish_packet(topic, payload, sizeof(payload) - 1, MQTT_RETAIN);
...
spotlight_netinit(void)
{
	...
	// init base topic
	mqtt_subscribe_set_topic = malloc(strlen(spotlight_params_ram.mqtt_topic) + strlen_P(PSTR(MQTT_SUBSCRIBE_SET_SUFFIX)) + 1);
	mqtt_subscribe_set_topic[0] = '\0';
	strncpy(mqtt_subscribe_set_topic, spotlight_params_ram.mqtt_topic, strlen(spotlight_params_ram.mqtt_topic) + 1);
	strcat_P(mqtt_subscribe_set_topic, PSTR(MQTT_SUBSCRIBE_SET_SUFFIX));

	mqtt_subscribe_set_format = malloc(strlen(spotlight_params_ram.mqtt_topic) + strlen_P(PSTR(MQTT_SUBSCRIBE_SET_FORMAT)) + 1);
	mqtt_subscribe_set_format[0] = '\0';
	strncpy(mqtt_subscribe_set_format, spotlight_params_ram.mqtt_topic, strlen(spotlight_params_ram.mqtt_topic) + 1);
	strcat_P(mqtt_subscribe_set_format, PSTR(MQTT_SUBSCRIBE_SET_FORMAT));

	mqtt_strobo_topic = malloc(strlen(spotlight_params_ram.mqtt_topic) + strlen_P(PSTR(MQTT_STROBO_SUFFIX)) + 1);
	mqtt_strobo_topic[0] = '\0';
	strncpy(mqtt_strobo_topic, spotlight_params_ram.mqtt_topic, strlen(spotlight_params_ram.mqtt_topic) + 1);
	strcat_P(mqtt_strobo_topic, PSTR(MQTT_STROBO_SUFFIX));

	mqtt_will_topic = malloc(strlen(spotlight_params_ram.mqtt_topic) + strlen_P(PSTR(MQTT_WILL_TOPIC_SUFFIX)) + 1);
	mqtt_will_topic[0] = '\0';
	strncpy(mqtt_will_topic, spotlight_params_ram.mqtt_topic, strlen(spotlight_params_ram.mqtt_topic) + 1);
	...
	if (dmx_conn_id != -1) {
		dmx_connected = true;
		SPOTDEBUG("Connection to dmx-storage established! id:%d",
                dmx_conn_id);
	}
	else {
		dmx_connected = false;
...
spotlight_main(void)
{
	if (update == SPOTLIGHT_UPDATE) { /*Only transmit if at least one channels has been updated */
